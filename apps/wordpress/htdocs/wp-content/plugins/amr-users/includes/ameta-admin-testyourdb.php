...
global $ausersadminurl;		
	...
	echo '<li>';
		printf(__('Php maximum execution time is: %s ', 'amr-users'),$max_time); 
	echo '</li>';
	echo '<li>';
		printf(__('Wordpress Memory limit: %s ', 'amr-users'),WP_MEMORY_LIMIT); 
	echo '</li>';
	echo '<li>';
		printf(__('Wordpress Max Memory limit: %s ', 'amr-users'),WP_MAX_MEMORY_LIMIT); 
	echo '</li>';
	echo '<li>';
...
*/		
		...
		$total = count($results);
		printf('<p><b>'.__('%s different user meta keys.', 'amr-users').'</b></p>',number_format($total,0,'.',',')); 
...
*/	
	...
	echo $before;
	$text2 = '<b>'.sprintf($text,number_format($total,0,'.',',')).'</b>';
	echo $text2;
