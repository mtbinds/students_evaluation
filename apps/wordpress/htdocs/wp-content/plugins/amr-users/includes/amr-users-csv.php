...
function amr_csvlines_to_csvbatch ($lines, $ulist=1) { // convert array to csv separated stuff
global $aopt;
...
global $amain;
	...
	if (empty($amain['public'][$ulist])) { // if it is not public - no file
...
global $amain;
	...
	//allow in front end if user is allowed
	//check_admin_referer('amr-meta');
	...
	else { //we are going to  have all the overhead of a webpage like this - prhaps one should treat more like a feed, and just issue the data ?
		$html = ausers_form_start();
		$html .= amr_generate_csv($ulist, true, false,'csv','"',',',chr(13).chr(10), $tofile );
		//$html .= amr_csv_form('csv', 'csv');
		$html .= ausers_form_end();
...
function amr_csv_form($csv, $suffix) {
	...
	return (
		...
		.  '<input type="submit" name="reqcsv" value="'
		.$text.'" class="button button-primary user-export" />' //20170907
		);
}
...
global $amain;
	...
	else { // should the form go here? no..
		if (current_user_can('list_users')) {
		$html = PHP_EOL.'<div class="csvlink">
		<p><a class="csvlink" title="'.$title.'" href="'.add_query_arg('csv',$ulist).'">'
		.$text
		.'</a></p>'.PHP_EOL.
		'</div><!-- end csv link -->'.PHP_EOL;
		}
