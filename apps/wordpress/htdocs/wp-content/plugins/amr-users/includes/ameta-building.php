...
// just do simply for now, as we have filtering later to chop out bits
	...
	//add other criteria 
	$where = apply_filters('amr-users-users-where',$where1);  // 20150820  NOtinUse? If is, it must be 'prepared' 

	//$query = $wpdb->prepare( "SELECT * FROM $wpdb->usermeta".$where); // WHERE meta_key = %s", $meta_key );
	$query = "SELECT umeta_id, user_id, meta_key, meta_value FROM $wpdb->usermeta "
	.$where;
