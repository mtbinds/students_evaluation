...
function amrmeta_validate_nicenames()	{
	global $amr_nicenames;
		...
		$ftypes = array(); 
		if ((isset($_POST['ftype'])) and (is_array($_POST['ftype']))) {
			foreach ($_POST['ftype'] as $i => $v) {
				if ($v) $ftypes[$i] = esc_attr($v); 
			}
		}
		
		ausers_update_option('amr-users-field-types', $ftypes);
...
function ameta_list_nicenames_for_input($nicenames) {
	...
	$fields_types_fixed = amr_fixed_fieldstypes(); //20170912 stop people messing up because they think its a format option
	...
	/* get the standard names and then the  meta names  */
		...
		if (!($fields_types = ausers_get_option('amr-users-field-types')))
			$fields_types = $fields_types_defaults;
		
		
		$fieldtypes = amr_fieldtypes();	//the generic list
		...
		ksort($nicenames);	
		//force excluded fields to the bottom
		foreach ($nicenames as $n => $nn) {
			if (empty($excluded[$n])) $sorttotop[$n] = true;
		}
		$nicenames =  auser_sortbyother($nicenames, $sorttotop);
		...
		.'</h2>'
		.'<p><a href="'.admin_url('admin.php?page=ameta-admin-meta-keys.php').'">'
		...
		.'</th>'
		.'<th>'
		.__('Exclude from all reports?','amr-users')
		.'</th>'
		.'<th>'
		.__('Field type','amr-users').'<br/>'.__('(Data type for format options)','amr-users')
		.'</th>'
		...
		foreach ($nicenames as $i => $v ) {
			if (!empty($excluded[$i])) $class = ' class="alternate striped" ';
			else $class = '';
			echo "\n\t".'<tr'.$class.'>'
			.'<td><label for="nn'.$i.'" >'.$i.'</label></td>';			
			
			echo '<td><input type="text" size="40" id="nn'.$i.'"  name="nn['.$i.']"  value= "'.$v.'" />';
			echo '</td>';


			echo '<td><input type="checkbox" id="wp'.$i.'"  name="wp['.$i.']"';
			...
			$ftype='';
			$disabled = '';
			if (!empty($fields_types_fixed [$i])) {
					$ftype = $fields_types_fixed [$i];
					$disabled = ' disabled="disabled" ';
					}
			elseif (!empty($fields_types[$i]) ) { //20170912 default fieldtypes must not be changed, can only add new
					$ftype = $fields_types[$i];				
			}	
							
			echo '<td><select id="fieldtype'.$i.'" '
			.' name="ftype['.$i.']" '.$disabled.' >';
			echo '<option value=""></option>';
			foreach ($fieldtypes as $fti => $ft ) {
				 echo ' <option value="'.$fti.'" ';
				 
				 if (!empty ($ftype) and ($ftype === $fti ))  
...
// style="background-image: url(images/screen-options-right-up.gif);"
	...
	$html = '<ol>'
	.'<li>'.__('Please exclude any meta keys you do not need. ', 'amr-users').__('Some plugins do weird things in their meta value content. ', 'amr-users')
	.'</li>'
...
/*  get all user data and attempt to extract out any object values into arrays for listing  */
	...
	if (!is_network_admin()) {	
		$keys['comment_count']	='comment_count';
		$keys['post_count']		='post_count';
	...
	//echo '<h3>'.sprintf(__('You have %s main user table fields', 'amr-users'),count($all)).'</h3>';
		...
		foreach ($all as $i2 => $v2){	
			if (!amr_excluded_userkey($v2) ) {
				...
				//echo '<br />'.__('Excluded:', 'amr-users').' "'.$v2.'"';
			}
		}
		/* Do the meta first  */
	if (!($excluded_meta_keys = ausers_get_option('amr-users-excluded-meta-keys')))  
	...
	$text = array();	//20170812
	foreach ($excluded_meta_keys as $key=>$on) {  // work out which not to bother with
		if ($on) $text[] = $key;
	}
	$string = "'".implode($text,"','")."'";
...
global $wpdb, $orig_mk;
	...
	if (!$orig_mk = ausers_get_option('amr-users-original-keys')) // the record of where the fields came from
		$orig_mk = array();
...
//	print_r ($all);
	...
	_e('Permanently exclude some: ', 'amr-users');
	foreach ($all as $i2 => $v2) {
		if (amr_permanently_excluded_metakeys($v2['meta_key']) ){
			unset ($all[$i2]);
			echo '.';
		}
	}
				...
				if (!empty($mv)) {
						...
						foreach ($temp as $i3 => $v3) {
							...
							if (is_array($v3) and function_exists('amr_dig_deeper')) { // *** needs work still
								...
								if (!isset($orig_mk[$mkey])) {
									$orig_mk[$mkey] = $mk;
									echo '<br />'.__('Added complex meta to report DB: ','amr-users').$mkey.'<br />';
								}
								else {
									//echo ' &#10003;'.$mkey;
								}
					...
					//if (!isset ($orig_mk[$key])) {
						...
						if (!isset($orig_mk[$mk])) {
							...
							//echo '<br />'.__('Added to report DB: ','amr-users').$mk.'<br />';
						}
						//else echo ' &#10003;';					
					}
				}
...
// use a field that is always there and has the table prefixes
	...
	$checkfield = 'capabilities';

	foreach ($nn as $i=> $n) {
		if (stristr($i, $checkfield)) {
			$prefixes_in_use[] = substr($i, 0, strpos($i,$checkfield)); //str_replace($checkfield, '', $i);
		}
	}
...
function amr_meta_nice_names_page() {
	...
	// NB do not check the values of submit fields as they may be translated.
	...
	if (isset($_POST['resetnice'])) { 
		if (ausers_delete_option ('amr-users-nicenames')) 
			_e('Deleting all nice name settings in database','amr-users');
		echo ' ';
		if (ausers_delete_option ('amr-users-nicenames-excluded')) 
			_e('Deleting all nice name exclusion settings in database','amr-users');
		echo ' ';	
		if (ausers_delete_option ('amr-users-original-keys')) 
			_e('Deleting original keys mapping in database','amr-users');
		echo ' ';
		if (ausers_delete_option ('amr-users-field-types')) 
			_e('Deleting field type specifications in database','amr-users');
		echo '<br />';			
	}
	if (isset($_POST['rebuild']) or isset($_POST['resetnice'])) {
				$amr_nicenames = ameta_rebuildnicenames ();
				_e('Rebuild Complete.', 'amr-users'); 
				return;
		}
	else {
