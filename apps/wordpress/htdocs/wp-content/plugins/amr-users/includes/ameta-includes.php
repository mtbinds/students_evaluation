...
function amru_addScheme($url, $scheme = 'http://')
{
  return parse_url($url, PHP_URL_SCHEME) === null ?
    $scheme . $url : $url;
}

function amru_localised_formatted_datetime ($dt, $format) { 
// takes a dt or just d string assumes utc, converts for tz using amru_newDateTime
// then to use wordpresses localisation, converts to timestamp, adjusted by offset for timezone
	$dto = amru_newDateTime($dt); //website timezone will be set
	// damn localisation requires timestamp, so have to adjust timestamp by the offset
	if (!is_object($dto)) 
		return ($dt.' !');
	$timestamp = $dto->format('U') + $dto->getOffset();
	$text = date_i18n($format,$timestamp);
	return $text;		
}

function amru_new_DateTime_nonfatal ($datetimestring='', $tzobj) { // date time create with exception trap to avoid fatal errors
		try {	
			$dt = new DateTime($datetimestring); 
			$dt->setTimezone($tzobj);
			}
		catch(Exception $e) {
			if (current_user_can('manage_options') or current_user_can('manage_userlists')) {
			echo '<p>'.$e->getMessage().'</p>';  // 2017 08 18 get actual message
			echo '<p>'
			.__('Message to admin only:','amr-users').' '
			.__('Fatal error caught','amr-users').' - '.
			sprintf(__('Unable to create DateTime object from this string: "%s" ','amr-users'),$datetimestring).' '
			.__('Please check the content of your date fields, and any fields where you have specified a date format','amr-users').'</p>';
			}
			//else fail silently
			return (false);
		}
...
function amru_newDateTime($text='') {  // create new datetime object with the global timezone
// because wordpress insists that the php default must stay as UTC
global $tzobj;
	if (empty($tzobj)) 
		$tzobj = amr_getset_timezone ();
	$d = amru_new_DateTime_nonfatal($text,$tzobj);  // traps exceptions
	return ($d);
}

function amru_get_func_name ($fld, $format) {
		global 	$amr_users_field_types;
	
		if (empty($amr_users_field_types)) // to get the type of field
			$amr_users_field_types = ausers_get_option('amr-users-field-types');
		if (empty($amr_users_field_types[$fld])) 
			return false; // no defined fieldtype
		else 
			$type = $amr_users_field_types[$fld];

		$func = 'ausers_format_'.$type.'_'.$format;
		return $func;
}
...
function amr_debug() {
	if ((WP_DEBUG or !empty($_REQUEST['debug']))  and is_user_logged_in() and current_user_can('activate_plugins')) {
		return true;
	}
	else return false;
}
...
function amr_remove_grouping_field ($icols) { // note cannot group and filter on the same field
...
global $amain, $aopt, $amr_current_list;
	...
	// 20170217 index must come before grouping, not just numerically															
	...
	// this is messing up the use of ID #20170201
	// Not anymore #2017017 ID seems to be ok now
...
function amr_users_can_edit ($type) {
		if (is_admin() and (current_user_can('manage_options') or current_user_can('manage_userlists') )
		and  (!empty($_REQUEST[$type]) or !empty($_REQUEST['config'])		))
		return true;
		else return false;
}
...
global $amr_current_list;
	...
	$html .= PHP_EOL.'<input type="hidden" name="ulist" value="'.$amr_current_list.'" />';
...
global $wpdb,$wp_version ;
		...
		'display_name',
		'user_status',
		'user_activation_key');	// unlikey to use for selection normally?	
...
function amr_users_dropdown ($choices, $current) { // does the options of the select
	...
	foreach ($choices as $opt => $value){	
		...
		if ($opt === $current) echo ' selected="selected" ';
