...
{	global $wpdb;
	...
	function record_cache_start ($reportid, $name) {
		$status = ausers_get_option ('amr-users-cache-status');
		//echo '<pre>rec start: ';print_r($status);echo '</pre>';
		
		if (empty($status) )
			$status = array(); //2018 05 30 logic was overwriiten a report each time
		else {
			if (isset($status[$reportid])) 
				unset ($status[$reportid]); //2018 05 15
			else 
				$status[$reportid] = array();
		}
	...
	if (is_object($d)) {
		...
		$timetext.' '.timezone_name_get($tzobj),
	...
	// note search text has been sanitised
		...
		if (($s[0] == '"') AND ($s[strlen($s) - 1] == '"'))  {  
			...
			foreach ($s as $i => $word) {
				...
				//$s[$i] = '  csvcontent LIKE "%'.$word.'%"  '; //20180315
				$args[] = $word;
				$s[$i] = '  csvcontent LIKE "%%%s%%" ';
			}
			$likes = '('.implode (' OR ', $s).')';
