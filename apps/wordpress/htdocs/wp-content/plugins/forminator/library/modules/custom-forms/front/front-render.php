...
class Forminator_CForm_Front extends Forminator_Render_Form {
	...
	public function enqueue_form_scripts( $is_preview, $is_ajax_load = false ) {
		...
		if ( $this->has_captcha() ) {
			...
			$language      = Forminator_Field::get_property( 'language', $first_captcha, $language );
	...
	public function pagination_start() {
		...
		$html = sprintf( '<div class="forminator-pagination forminator-pagination-start" data-step="0" data-label="%1$s">', $label );
	...
	public function pagination_step( $step, $field ) {
		...
		$html = sprintf( '</div><div class="forminator-pagination" data-step="%1$s" data-label="%2$s">', $step, $label );
...
//		$html .= $this->get_description( $field );
	...
	public function get_button_markup() {
		...
		if ( $this->get_form_design() !== 'material' ) {
			$html .= sprintf( '<button id="forminator-submit" class="' . $class . '">%s</button>', $button );
		} else {
			$html .= sprintf( '<button id="forminator-submit" class="' . $class
			                  . '"><span class="forminator-button--mask" aria-label="hidden"></span><span class="forminator-button--text">%s</span></button>',
			                  $button );
		}
		$html .= '</div>';
		$html .= '</div>';
		...
		if ( ! empty( $style_properties ) ) {
			...
			foreach ( $style_properties as $style_property ) {
				...
				if ( isset( $properties['form_id'] ) && strlen( $trimmed_styles ) > 0 ) {
					?>
					<style type="text/css" id="forminator-custom-form-styles-<?php echo esc_attr( $properties['form_id'] ); ?>">
	...
	public function get_html( $hide = true, $is_preview = false ) {
		...
		if ( $this->model->form_is_visible() ) {
			...
			if ( isset( $form_settings['expire_message'] ) && '' !== $form_settings['expire_message'] ) {
				...
				<div class="forminator-custom-form"><label class="forminator-label--info"><span><?php echo esc_html( $message ); ?></span></label></div>
