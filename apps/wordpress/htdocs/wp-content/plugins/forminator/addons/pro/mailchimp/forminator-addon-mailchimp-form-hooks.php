	...
	public function add_entry_fields( $submitted_data ) {
		...
		try {
			...
			foreach ( $mailchimp_required_fields as $mailchimp_required_field ) {
				...
				if ( ! isset( $submitted_data[ $addon_setting_values['fields_map'][ $mailchimp_required_field->tag ] ] )
				     || empty( $submitted_data[ $addon_setting_values['fields_map'][ $mailchimp_required_field->tag ] ] ) ) {
					throw new Forminator_Addon_Mailchimp_Exception(
						sprintf(
							__(
			...
			foreach ( $mailchimp_fields_list as $item ) {
				...
				if ( isset( $addon_setting_values['fields_map'][ $item->tag ] ) && ! empty( $addon_setting_values['fields_map'][ $item->tag ] ) ) {
					if ( isset( $submitted_data[ $addon_setting_values['fields_map'][ $item->tag ] ] ) && ! empty( $submitted_data[ $addon_setting_values['fields_map'][ $item->tag ] ] ) ) {
						$merge_fields[ $item->tag ] = trim( $submitted_data[ $addon_setting_values['fields_map'][ $item->tag ] ] );
					}
				}
