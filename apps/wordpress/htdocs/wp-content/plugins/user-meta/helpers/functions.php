...
/**
 * Get messages from transient originated https://user-meta.com
 *
 * @return array
 */
function getRemoteMessages()
{
    return get_site_transient('user_meta_remote_messages');
}

/**
 * Retrieve messages from https://user-meta.com
 */
function retrieveRemoteMessages()
{
    global $userMeta;
    try {
        $url = $userMeta->website . '/wp-json/api/v1/promotional-message/';
        $offer = wp_remote_retrieve_body(wp_remote_get($url));
        $offer = json_decode($offer, true);
        if ($offer)
            set_site_transient('user_meta_remote_messages', $offer, 60 * 60 * 24);
    } catch (\Exception $e) {}
}