...
use MailOptin\Core\Admin\Customizer\CustomControls\WP_Customize_Autocomplete_Input_Control;
...
class Connect extends AbstractConvertFoxConnect implements ConnectionInterface
{
    ...
    {
        $connections[self::$connectionName] = __('Gist (ConvertFox)', 'mailoptin');
    ...
    public function integration_customizer_controls($controls)
    {
        if (defined('MAILOPTIN_DETACH_LIBSODIUM') === true) {
            ...
            $content = sprintf(
                __("%sMailOptin Premium%s allows you to apply tags to leads and record custom conversion events to ConvertFox.", 'mailoptin'),
                '<a target="_blank" href="https://mailoptin.io/pricing/?utm_source=wp_dashboard&utm_medium=upgrade&utm_campaign=convertfox_connection">',
                '</a>',
                '<strong>',
                '</strong>'
            );
