...
LEGO_BIN=/opt/wordpress/letsencrypt/lego
CERTIFICATES_DIR=/opt/wordpress/letsencrypt/certificates
...
is_apache_installed() {
    if [ -d "/opt/wordpress/apache2" ]; then
        true
    else
        false
    fi
...
stop_server() {
    /opt/wordpress/ctlscript.sh stop "$(server_type)"
}

start_server() {
    /opt/wordpress/ctlscript.sh start "$(server_type)"
}

restart_server() {
    /opt/wordpress/ctlscript.sh restart "$(server_type)"
}

is_varnish_enabled() {
    if [[ -f "/opt/wordpress/varnish/scripts/ctl.sh" ]]; then
        true
    else
        false
    fi
...
is_varnish_running() {
    if is_varnish_enabled && "/opt/wordpress/ctlscript.sh" status varnish 2> /dev/null | grep "already running" > /dev/null 2>&1 ; then
        true
    else
        false
    fi
...
stop_varnish() {
    "/opt/wordpress/ctlscript.sh" stop "varnish"
}

start_varnish() {
    "/opt/wordpress/ctlscript.sh" start "varnish"
}
...
web_server_conf_dir() {
    if is_apache_installed; then
        echo "/opt/wordpress/apache2/conf"
    else
        echo "/opt/wordpress/nginx/conf"
    fi
}
...
modify_configuration() {
    if is_apache_installed; then
        sed -i "s;\s*SSLCertificateFile\s.*;  SSLCertificateFile \"/opt/wordpress/apache2/conf/${domains[0]}.crt\";g" "$(web_server_conf_dir)/bitnami/bitnami.conf"
        sed -i "s;\s*SSLCertificateKeyFile\s.*;  SSLCertificateKeyFile \"/opt/wordpress/apache2/conf/${domains[0]}.key\";g" "$(web_server_conf_dir)/bitnami/bitnami.conf"
    else
...
EOF
    ...
    if is_apache_installed; then
        server_restart_command="/opt/wordpress/apache2/bin/httpd -f /opt/wordpress/apache2/conf/httpd.conf -k graceful"
    else
        server_restart_command="/opt/wordpress/nginx/sbin/nginx -s reload"
    fi

    crontab $USER -l 2> /dev/null | {
        cat
        echo "0 0 1 * * $SUDO $LEGO_BIN --path=\"/opt/wordpress/letsencrypt\" --tls --email=\"${email}\" ${domain_args} renew && $SUDO $server_restart_command"
...
EOF
    ...
    "$LEGO_BIN" --path "/opt/wordpress/letsencrypt" --tls --email="${email}" ${domain_args} run
